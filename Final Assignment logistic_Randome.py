...
# Now we build the RBF Model using Optimal-C and Optimal-G

m_rbf=svm.SVC(kernel="rbf",C=opt_C,gamma=opt_G).fit(trainx,trainy)
p_rbf=m_rbf.predict(testx)

 # Confusion Matrix and Classification report of RBF Model
 print(cm(list(testy),list(p_rbf)))
 testy.value_counts()
 print(cr(testy,p_rbf))
 print("Accuracy of RBF Model is: ",accuracy_score(testy,p_rbf)*100)
